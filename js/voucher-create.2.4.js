/**
 * Created by xqs on 2016/3/28. Last modify on 2016/10/19
 * voucher-create.js
 * 优惠券创建页面
    ...
    var page = {
        ...
        optimize: function () {
            ...
            if (!c.isCreate && !c.canUpdate) {  //查看模式
                ...
                _form.find(':text,textarea').not('[name="landingDesc"],[name="landingDetail"]').attr('readonly', true);
                _form.find(':radio,:checkbox,select').not('[name="landingType"]').attr('disabled', true);
            }
        ...
        bindEvents: function () {
            ...
            /*单张券类型 0:金额  1:折扣*/
            $(document.body).on('change', '[name="decreaseType"]', function(){
                if(+$(this).val() === 0){
                    $('.js-amount').show();
                    $('.js-discount').hide();
                }else{
        ...
        addTpl: function () {
            ...
            $.each(tpl.find('.datepicker'), function () {
                ...
                _this.attr('onfocus', "WdatePicker({dateFmt:'yyyy-MM-dd'," + limit + ":'#F{$dp.$D(\\'" + _target + index + "\\',{s:-1});}'});");
            });
        ...
        validate: function (container) {
            ...
            $.each(_validate_ele, function () {
                ...
                // isDecimal
                if (_this.data('type') === 'float' && !isForm.isDecimal(this_val,1)) {
                    !_this.hasClass('datepicker') && _this.focus();
                    Toast.show(tips + '只允许输入一位小数！');
                    return check_res = false;
                }
        ...
        getFormData: function () {
            ...
            switch (c.voucherType) {
                ...
                case 1:   //优惠券相同数据部分
                    ...
                    if(c.voucherType == 1) extData.disPlayStyleNumIds = $.trim($('[name="disPlayStyleNumIds"]').val()).split(',')
                    ...
                    $.each($('.ticket-box'), function () {
                        var _this = $(this);
                        var itemData = {
                            "denominationAmount": _this.find('.amount:visible').val() * 100, //面额（分）/折扣
                        ...
                        if(c.voucherType == 0) {
                            itemData.decreaseType = +$('[name="decreaseType"]').val();
                        }
