  ...
  var HOME_CONFIG = {
    config: {
      ...
      sourceTypes: {
        ...
        { "bannerType": 5, "limit": 10, "sort": 0, "name": "图标ICON", "picSize": ["105x105"] },
    ...
    init: function(op) {
      ...
      console.log({"当前页面可配资源位": c.srcTypes });
      self.event();   // 绑定时间
      self.render();  // 渲染页面
    },
    /* 绑定所有事件 */
    event: function() {
      ...
      o.srcContainer.on('click', '.js-module-online', function() {self.fn_upModule(this); }); // 二级导航 选择模块

      o.adsContainer.on('click', o.showConfigBtn, function() {self.fn_showConfig(this); });   // 投放页 绑定显示具体广告配置点击
      $('select[name="linkType"]').on('change', self.fn_changeLinkType);                      // 切换落地类型

      $('input[name="linkTo"]').on('change', function() { // 填写妈豆商品后
      ...
      o.saveConfigBtn.on('click', function(){self.fn_saveConfig(this)}); // 保存具体广告配置
      o.adsContainer.on('click', '.up', function() {self.fn_move(0, this); }); // 上移资源位
      o.adsContainer.on('click', '.down', function() {self.fn_move(1, this); }); // 下移资源位
      // 切换轮播banner
      ...
      // 预览
      o.previewBtn.on('click',function(){self.fn_preview(this)});
      // 排序
      o.reSortBtn.on('click',function(){self.fn_reSort(this)});

      o.adsContainer.on('click', '.js-px' , function(){self.fn_setSpacePx(this);});
              ...
              if ([3, 4, 5, 14, 25].indexOf(group.bannerType) > -1) {
                ...
                $.each(bannerGroup, function(j, banner) {
                  if (banner.linkType == 10) {
                    ...
                    var nav0Html = [];
                    nav0Html.push('<td><span>门店</span></td> <td><span>999</span></td> <td><a href="javascript:;" class="btn btn-info btn-mini js-update-status">' + (banner.isValid ? '隐藏' : '显示') + '</a></td><td></td><td></td>');
                    /*if (banner.pic) {
    ...
    fn_preview:function(obj){
      var self = this, c = self.config, _this = $(obj), arr = [], items = [], date = _this.closest('.MA-putin-block').data('date');
    ...
    fn_reSort:function(obj){
      var self = this, c = self.config, _this = $(obj), toDate = _this.closest('.MA-putin-block').data('date'),
    ...
    fn_setSpacePx:function(obj){
      var self = this, c = self.config, o = self.eles, _this = $(obj),_item = _this.closest('.js-item');
    ...
    fn_saveSource: function(obj) {
      ...
      // return console.log([reqData,bannerGroup]);
      console.log(reqData);
      Matrix.JSON({
        showLoad: true,
        type: "POST",
    ...
    fn_cleanModal: function(modal) {
      ...
      if (bannerType == 14) {
        ...
        if (modal.find('input[name="isAll"]:checked').val() == 1) {
        } else {
    ...
    getSrcObjById:function(id){
      var self = this, c = self.config, srcObj = {};
...
fn_saveConfig: function(obj) {
  var self = this, c = self.config, _frm = $(obj).closest('.modal');
    ...
    UIShot: {
      ...
      */
      buildItem: function(type, data) {
        var self = this, c = self.config, htmlArr = [],
        dom = null,
        item = null,
        ...
        if(type == 3 || type == 4){
          ...
          htmlArr.push('<div class="move-title"><h2>' + (groupName || typeObj.name) + '- 间隔' + px + 'px</h2><div class="move-btn"><span class="js-px">间隔</span></div></div>');
          htmlArr.push('</ul>');
        }else if(type == 14){
          htmlArr.push('<div class="swiper"></div>');
        }else{
          htmlArr.push('<div class="move">');
          htmlArr.push('<div class="move-title"><h2>' + (groupName || typeObj.name) + '- 间隔' + px + 'px</h2><div class="move-btn"><span class="js-px">间隔</span> <span class="up">上移</span> <span class="down">下移</span></div></div>');
          htmlArr.push('</div>');
        }
        ...
        $.each(dom.find('.tools'),function(i,o){
          var b = banners[i], _this = $(o), bannerName = b.bannerName;
          if(!bannerName){
            // 用于弹窗显示
            bannerName = banners.length > 1 ? groupName + ' - 坑位' + (i+1) : groupName;
          }
          // 用于保存具体广告时获取
          _this.data({'id':b.id || '','banner-id':b.bannerId,'banner-name':bannerName,'weight':b.weight});
          // 判断上下线状态
          if(isOnline){
            // 判断是否已配置具体广告
            if(b.id){
       ...
       */
       buildSourceItem:function(srcObj){
        var htmlArr = [], dom = null, srcType = srcObj.bannerType, data = srcObj || null, limit = srcObj.limit;
        var colspan = 2, moduleType = HOME_CONFIG.config.moduleType;
        ...
        if(limit == undefined){
          ...
          dom = $(htmlArr.join(''));
          if(data){
            dom.data({'banner-id':data.bannerId,'batch-id':data.batchId,'isvalid':data.isValid});
          }
          ...
          if ([0, 1, 2, 23].indexOf(srcType) > -1) {
            ...
            if (srcType == 3) {
              // 一级导航/二级导航可配置图片/排序
              htmlArr.push('<thead> <tr> <th width="220">名称</th> <th width="100">排序</th> <th>操作</th> <th>图片配置</th><th>6p图片配置</th></tr> </thead>');
              colspan = 5;
            ...
            htmlArr.push('<tbody>');
            if (srcType == 3) {
              htmlArr.push('<tr> <td><span>门店</span></td> <td><span>999</span></td> <td><a href="javascript:;" class="btn btn-info btn-mini js-update-status">显示</a></td> <td></td><td></td> </tr>');
            }
