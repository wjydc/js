    ...
    fn_saveConfig: function(obj) {
      ...
      if (bannerType == 14) { // 人群定向 applyScope {isAll,prvId,cityId,areaId}
        ...
        if (_frm.find('[name="isAll"]:checked').val() == "") {
          if(_frm.find('[name="prvId"]').val() == '' || _frm.find('[name="cityId"]').val() == '' || _frm.find('[name="areaId"]').val() == ''){
            Toast.show('区域定向请填写完整');
            validate = false;
          }
          applyScope = {
      ...
      */
      buildItem: function(type, data) {
        ...
        if(type == 3 || type == 4){
          ...
          htmlArr.push('<ul class="icon"></ul>');
        }else if(type == 14){
          htmlArr.push('<div class="swiper"></div>');
        }else{
          htmlArr.push('<div class="move">');
          htmlArr.push('<div class="move-title"><h2>' + (groupName || typeObj.name) + ' -【' + data.batchId + '】</h2><div class="move-btn"><span class="up">上移</span> <span class="down">下移</span></div></div>');
          htmlArr.push('</div>');
        }
