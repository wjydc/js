var Setting = {
	...
	event: function(){
		...
		// 类目切换;
		o.goodsList.on("change", "select", function(){
		...
		// 删除模版;
		o.goodsList.on("click", ".js-del-item", function(){
	...
	template: function(){
		...
		// 类目数据初始化;
		var items = o.goodsList.find(".item"), i = 0, l = items.length;
		if(l){
			for(; i < l; i++){
	...
	// 商品模版;
	items: function(){
		var self = this, o = self.info, d = self.data, c = self.config, arr = [];
		for(var i = 0; i < d.Items.length; i++){
			arr.push(self.itemhtml(i, d.Items[i]));
		}
		return arr.join('');
	},
	// 组装商品模版;
	itemhtml: function(i, d){
		...
		arr.push('<p><label for="price-' + i +'diy"><input type="radio" name="price-'+ i +'" value="-1" id="price-' + i +'diy" name="price" '+ (c.priceDiy ? "checked" : '') +' /> 自定义<input type="text" style="width:50px;" value="'+ (d && c.priceDiy ? d.Price : '') +'" />元以上</label></p>');
		arr.push('</dd></dl></div>');
		return arr.join('');
	},
	// 品牌;
	...
	seve: function(){
		var self = this, o = self.info, c = self.config;
		// 指定商品 id;
		var ids = o.goodsId.val().trim().replace(/\，/g, ',');
		ids = ids.split(",");
		if(ids != ""){
			if(ids[ids.length-1] == ""){
		...
		var items = o.goodsList.find(".item"), i = 0;
		for(; i < items.length; i++){
			...
			if(price.val() == -1){
				...
				if(!isForm.isImp(obj.Price)){ return Toast.show('请填写正确的自定义价格');}
			}else{
				obj.Price = price.val();
			}
		...
		//return console.log(JSON.stringify(self.data));
